...
export default function BigChartBox() {
  ...
  const totalAppointmentsInWeek = new Array(7).fill(0).map((_, index) => {
    ...
    const totalAppointments = patientsData?.filter((patient: any) => {
      ...
      return (
        ...
        (selectedDepartment === "" ||
          patient.doctor.depID === selectedDepartment)
      );
    });
  ...
  const filterday = isSmallScreen
    ? daysOfWeek.filter((day, index) => {
        if (
          totalAppointmentsInWeek[index].uv > 0 &&
          (new Date().getDay() + 6) % 7
        ) {
          return day;
        }
    ...
    <div className="h-[100%] w-[100%] flex flex-col justify-between">
      ...
      <h1 className="text-center mt-2">
        ...
        >
          ...
          {departments?.map((depart: any, index: number) => (
            <option key={index} value={depart.id.trim()}>
      ...
      <div className="h-[300px] w-[100%]">
        <ResponsiveContainer width="99%" height="100%">
          <BarChart width={500} height={300} data={totalAppointmentsInWeek}>
            <XAxis
            ...
            <Bar
              ...
              label={{ position: "top" }}
            >
              {totalAppointmentsInWeek.map((entry, index) => (
